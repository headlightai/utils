...
PointcloudFromLaserscan::PointcloudFromLaserscan() {
    yaml_dir_ = ament_index_cpp::get_package_share_directory("utils") + "/config/pointcloud_from_laserscan.yaml";
...
void PointcloudFromLaserscan::pointcloudFromLaserscan() {
    for (const std::string& f : bag_folders_) {
        ...
        for (const auto& ls : laserscans_) {
            ...
            oss << std::put_time(std::localtime(&sec), "%Y-%m-%d %H:%M:%S") << "." << std::setw(9) << std::setfill('0')
                << nsec;
